		...
		function drawBoard()
		{
			...
			cxt.fillRect(0,0,p_width,p_height);
			cxt.fillStyle="#444444";
			cxt.beginPath();
			cxt.moveTo(0,p_height);
			cxt.lineTo(0,0);
			cxt.lineTo(p_width,0);
			cxt.lineTo(p_width-inset_width,inset_width);
			cxt.lineTo(inset_width,inset_width);
			cxt.lineTo(inset_width,p_height-inset_width);
			cxt.closePath();
			cxt.fill();
		}
	...
	function plotData(){
	  ...
	  x_coords = new Array(86280,86281,86282,86283,86284,86285,86286,86287,86288,86289,86290,86291,86292,86293,86294,
		...
		var x_coord_max = x_coords.max();
		var x_coord_min = x_coords.min();
		var y_coord_max = y_coords.max();
		...
		{
			x = (x_coords[i]-x_coord_min)*(440/(x_coord_max-x_coord_min));
			y = (y_coords[i]-y_coord_max)*(-275/(y_coord_max - y_coord_min));		
			if(i==0){
				cxt.moveTo(x,y);
				continue;
			}
